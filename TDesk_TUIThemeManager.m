...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection {
    ...
    if (@available(iOS 13.0, *)) {
        if ([self.traitCollection hasDifferentColorAppearanceComparedToTraitCollection:previousTraitCollection]) {
            ...
            if ([TDeskThemeManager.shareManager respondsToSelector:@selector(allListenerExcuteonApplyThemeMethod:module:)]) {
                [TDeskThemeManager.shareManager performSelector:@selector(allListenerExcuteonApplyThemeMethod:module:) withObject:nil withObject:nil];
            }
        }
    }
}
...
@implementation TDeskTheme

+ (UIColor *)dynamicColor:(NSString *)colorKey module:(TUIThemeModule)module defaultColor:(NSString *)hex {
    TDeskTheme *theme = TUICurrentTheme(module);
    TDeskTheme *darkTheme = TUIDarkTheme(module);
    if (theme) {
 ...
 */
@property(nonatomic, strong) NSMutableDictionary<NSNumber *, TDeskTheme *> *currentThemeCache;

/**
 * The dark theme for each module, if any
 */
@property(nonatomic, strong) NSMutableDictionary<NSNumber *, TDeskTheme *> *darkThemeCache;

@property(nonatomic, strong) NSHashTable *listeners;

- (void)allListenerExcuteonApplyThemeMethod:(TDeskTheme *)theme module:(TUIThemeModule)module;

@end

@implementation TDeskThemeManager
#ifdef TUIThreadSafe
...
- (void)addListener:(id<TDeskThemeManagerListener>)listener {
#ifdef TUIThreadSafe
...
- (void)removeListener:(id<TDeskThemeManagerListener>)listener {
#ifdef TUIThreadSafe
    dispatch_async(_queue, ^{
#endif
      if ([self.listeners containsObject:listener]) {
...
- (void)registerThemeResourcePath:(NSString *)path darkThemeID:(NSString *)darkThemeID forModule:(TUIThemeModule)module {
    ...
    dispatch_async(_queue, ^{
      ...
      TDeskTheme *theme = [self loadTheme:darkThemeID module:module];
      if (theme) {
...
- (TDeskTheme *)currentThemeForModule:(TUIThemeModule)module {
    __block TDeskTheme *theme = nil;
#ifdef TUIThreadSafe
...
- (void)setCurrentTheme:(TDeskTheme *)theme forModule:(TUIThemeModule)module {
#ifdef TUIThreadSafe
...
- (TDeskTheme *)darkThemeForModule:(TUIThemeModule)module {
    __block TDeskTheme *theme = nil;
#ifdef TUIThreadSafe
    dispatch_sync(_read_write_queue, ^{
#endif
...
- (void)setDarkTheme:(TDeskTheme *)theme forModule:(TUIThemeModule)module {
#ifdef TUIThreadSafe
    dispatch_barrier_async(_read_write_queue, ^{
#endif
      if ([self.darkThemeCache.allKeys containsObject:@(module)]) {
...
- (void)doApplyTheme:(NSString *)themeID forSingleModule:(TUIThemeModule)module {
    TDeskTheme *theme = [self loadTheme:themeID module:module];
    if (theme == nil) {
        return;
    }
...
- (TDeskTheme *)loadTheme:(NSString *)themeID module:(TUIThemeModule)module {
    ...
    TDeskTheme *theme = [[TDeskTheme alloc] init];
...
- (void)notifyApplyTheme:(TDeskTheme *)theme module:(TUIThemeModule)module {
    if (theme == nil) {
        return;
    }
...
- (void)allListenerExcuteonApplyThemeMethod:(TDeskTheme *)theme module:(TUIThemeModule)module {
    for (id<TDeskThemeManagerListener> listener in self.listeners) {
