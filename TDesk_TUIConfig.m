...
+ (id)defaultConfig {
    ...
    static TDeskConfig *config;
    dispatch_once(&onceToken, ^{
      config = [[TDeskConfig alloc] init];
    });
    return config;
}
...
- (void)setSceneOptimizParams:(NSString *)path {
    ...
    NSDictionary *msgData =
        @{@"sdkappid" : @([TDeskLogin getSdkAppID]), @"bundleId" : NSBundle.mainBundle.bundleIdentifier ?: @"", @"package" : @"", @"component" : path};
    NSString* userId =[TDeskLogin getUserID];
