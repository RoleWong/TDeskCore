...
@implementation TDeskLoginConfig
...
@interface TDeskLogin () <V2TIMSDKListener>
...
@implementation TDeskLogin

#pragma mark - API
+ (void)initWithSdkAppID:(int)sdkAppID {
    [TDeskLogin.shareInstance initWithSdkAppID:sdkAppID];
}

+ (void)login:(NSString *)userID userSig:(NSString *)userSig succ:(TSucc)succ fail:(TFail)fail {
    [TDeskLogin.shareInstance login:userID userSig:userSig succ:succ fail:fail];
}

+ (void)login:(int)sdkAppID userID:(NSString *)userID userSig:(NSString *)userSig succ:(TSucc)succ fail:(TFail)fail {
    [TDeskLogin.shareInstance login:sdkAppID userID:userID userSig:userSig config:nil succ:succ fail:fail];
}

+ (void)login:(int)sdkAppID userID:(NSString *)userID userSig:(NSString *)userSig config:(TDeskLoginConfig *)config succ:(TSucc)succ fail:(TFail)fail {
    [TDeskLogin.shareInstance login:sdkAppID userID:userID userSig:userSig config:config succ:succ fail:fail];
}

+ (void)getSelfUserInfo {
    [TDeskLogin.shareInstance getSelfUserInfo];
}
...
+ (int)getSdkAppID {
    return [TDeskLogin.shareInstance getSdkAppID];
}
...
+ (NSString *)getUserID {
    return [TDeskLogin.shareInstance getUserID];
}

+ (NSString *)getUserSig {
    return [TDeskLogin.shareInstance getUserSig];
}

+ (NSString *)getNickName {
    return [TDeskLogin.shareInstance getNickName];
}

+ (NSString *)getFaceUrl {
    return [TDeskLogin.shareInstance getFaceUrl];
}

+ (void)setCurrentBusinessScene:(TUIBusinessScene)scene {
    TDeskLogin.shareInstance.currentBusinessScene = scene;
}

+ (TUIBusinessScene)getCurrentBusinessScene {
    return TDeskLogin.shareInstance.currentBusinessScene;
}
