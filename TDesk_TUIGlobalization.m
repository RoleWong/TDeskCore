...
@implementation TDeskGlobalization

+ (void)load {
    NSString *language = [TDeskGlobalization getPreferredLanguage];
    if([language hasPrefix:@"ar"]) {
        [TDeskGlobalization setRTLOption:YES];
    }
    else {
        [TDeskGlobalization setRTLOption:NO];
    }
}
...
+ (NSBundle *)private_mainBundle {
    ...
    NSString *customLanguage = [TDeskGlobalization getPreferredLanguage];
